...
6. After each file write or major edit, immediately reopen the file and confirm it is complete, untruncated, and syntactically/import-valid before building on it; treat malformed content, escaped newlines, partial writes, or suspicious truncation as blocking errors.
...
11. If any required metric or safety check fails, treat the task as unfinished: keep iterating or state explicitly which requirement remains unmet; do not present the task as successful while known failures remain.
