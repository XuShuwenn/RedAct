...
- Inspect the source definitions for the actual input/result data models used by the sequential pipeline before coding; match real field names, wrapper types, and object shapes instead of assuming them.
...
- Inspect adjacent supporting modules that define shared dataclasses, document objects, and result/index types before coding; confirm the exact document shape and reuse those existing models instead of inferring or recreating them.
- If the baseline depends on imported helpers or data definitions from other project files, read those definitions too before implementing the parallel path.
...
- Prefer importing and reusing the baseline module's public classes, dataclasses, helper functions, and result/index types directly when the environment allows it instead of duplicating their definitions in the parallel module.
...
- If direct reuse is impossible, keep wrappers/adapters minimal and preserve the baseline signatures and return types exactly so interface drift is easy to detect.
...
- Before any targeted patch, read enough surrounding code to see the full function/class/signature you will change in both the sequential baseline and the current target file. Patch only against exact observed text; do not use invented anchors like `existing helper`, `existing import block`, `previous implementation`, or `...`.
- If you have only seen a snippet of the baseline or target file, continue reading before editing; do not rewrite whole files from assumptions.
...
- Never save a `.py` file containing narrative summaries, TODO prose, or stand-in text. If you do not yet know the exact code, stop and inspect more of the actual source instead.
- After any overwrite or heredoc write to a source file, immediately read the saved region or whole file back and confirm it still contains coherent runnable Python.
...
- Before the first command, make a short protocol checklist from the observed task instructions: exact `Action:`/JSON syntax if required, allowed tools, required wait/observation pattern, concrete command argument format, and the exact final completion sentinel. Use only that format for the entire run; do not improvise alternate tool-call markup, malformed commands, shorthand variants, or near-miss completion lines.
- For every verification or benchmark step, issue a literal executable command with concrete arguments and paths; do not write placeholders such as "run the benchmark" or other non-runnable descriptions.
...
- Confirm the target module is importable in the current environment before deeper debugging or benchmarking; for repository-style tasks, check whether `PYTHONPATH` or the working directory must be set so local modules resolve correctly.
- Before editing, capture the exact compliance calls you must satisfy on the required workload and interface: specified benchmark entry points, required parameters such as `num_workers`/`chunk_size`, output shape, and any mandated completion signal. Optimize for those exact checks rather than nearby variants.
...
- Match the sequential contract exactly when returning results: preserve required wrapper/result object types, tuple structure, and function signatures before optimizing further.
...
- Treat deterministic merge order as a correctness requirement: after parallel chunk reduction, explicitly restore the sequential version's posting order, vocabulary or document ordering, and result tie-break behavior before comparison or return.
- If required-condition validation succeeds, emit the exact required completion sentinel and stop; do not keep tuning, rewriting, or starting extra benchmark cycles after a validated solution.
...
- Start from a correctness-first split that has worked well in practice: parallelize expensive independent per-document or per-query work, then keep lightweight corpus-wide aggregation and final deterministic assembly in the main process unless timings show those steps are true bottlenecks.
...
- Prefer worker-initialized shared read-only state for large immutable search or index data; dispatch only lightweight query or document slices or IDs so workers do not repeatedly receive the full index or document-frequency structures.
...
- When profiling indexing, distinguish at least tokenization or per-document term extraction, inverted-index or posting construction, DF or IDF aggregation, merge or final assembly, worker startup, and serialization so redesign effort stays focused on the true hot stages.
...
- Reuse project-defined `Document`, `SearchResult`, index or result dataclasses, and other shared models from the existing codebase rather than introducing parallel-only structural variants unless the task explicitly requires a new type.
...
- Before blaming multiprocessing itself, verify every pool target, initializer, and helper it references is importable at module scope and does not close over local state.
- If worker execution fails unexpectedly, check for nested functions, lambdas, bound local closures, or non-picklable task arguments before redesigning the architecture.
...
- Let each indexing worker do as much self-contained document work as possible per chunk—tokenization, local term frequencies, and partial posting or count preparation—before returning compact partial outputs for centralized merge.
- When indexing speedup is weak, first try narrowing the parallel region to the heaviest independent per-document preprocessing stage and keep aggregation, corpus-wide statistics, and final index assembly centralized; this often preserves exact semantics while avoiding coordination overhead.
...
- On Unix-like systems, when using process-based parallelism, prefer preparing large immutable search or index inputs before worker creation so forked workers can reuse that read-only state with minimal pickling; still keep a safe initializer-based path for environments that use spawn.
...
- For search, start with the simplest measured-good distribution: one query task per work item plus an appropriate `chunksize`, while the index or documents stay in worker-initialized read-only globals. If per-query work is too small to amortize overhead, batch chunks of queries per worker call instead; always preserve original query order when flattening results.
- Preserve the baseline API and result shapes while shrinking execution payloads: optimize transport and batching, not observable return structures.
...
- Use the baseline-preserving workflow in this order: inspect sequential interface, types, and helpers end-to-end, implement parallel orchestration around the existing logic, merge partial outputs deterministically, then validate by direct baseline comparison before optimizing further.
- Keep the parallel implementation anchored to the baseline module's existing interfaces and helper or data structures so equivalence checks are direct and low-risk.
- Profile the observed indexing or search pipeline by stage before broad rewrites; if tokenization or index or vector construction dominates while IDF or other corpus-wide computations are cheap, optimize the expensive stages first.
- Choose chunking, batching, and sequential-fallback thresholds from measured workload shape rather than assumptions.
- Use a simple escalation rule: first verify correctness, then measure stage costs, then redesign the dominant-cost stage. Only after that should you retune `num_workers`, `chunk_size`, batching, or fallback thresholds.
- Preserve a working subsystem once it already meets the target; do not refactor both indexing and search at once when measurements show only one subsystem is underperforming.
- Choose chunk sizes or batch sizes large enough to amortize pool overhead, and keep a sequential fallback for small document or query batches where multiprocessing would be slower.
...
- If no suitable checker exists, add a small temporary script or command sequence that compares sequential vs. parallel outputs and benchmarks both in one run; use it to catch correctness regressions before further optimization.
- Finish reading any paginated or truncated baseline or target file before implementing; an initial prefix is not enough to ground semantics-preserving parallelization.
- If multiprocessing breaks at runtime, inspect picklability first: pool targets should be top-level functions, initializers should be top-level, and task payloads should avoid closures or complex non-picklable objects.
- If indexing benchmarks already satisfy the required speedup, leave indexing mostly unchanged and spend iteration budget on search-specific overheads such as pool startup and repeated index serialization.
