...
If you create a script file, ensure the saved file contains executable Python, not a natural-language placeholder or summary of intended steps. Read the file back once before execution if there is any chance the write step produced truncated or non-code content.

Pre-run gate:
- The script file must contain runnable Python, not prose such as "this script loads the lightcurve and finds the period".
- The execution step must be a literal shell command, for example `python3 /root/find_period.py`, not a natural-language instruction.
- If either the script body or the command is still descriptive rather than executable, stop and fix that before attempting the run.

Never run a scaffold that contains only comments, docstrings, imports, or high-level step labels. First implement the concrete analysis path that reads the real lightcurve, computes a candidate, and writes `/root/period.txt`, then inspect the saved file, and only then execute it.

If your first data look was only a header or small sample, add one lightweight whole-file summary step before locking in the algorithm; do not choose the search approach from sample rows alone.
...
For `transitleastsquares` specifically, verify that you read period results from the search result returned by `power(...)`, not from the TLS model object itself. Wrong pattern: `model.period`. Right pattern shape: `results = model.power(...); candidate = results.period`.

If the first real TLS run is killed or stalls near startup, do not rerun the same broad search unchanged. Immediately reduce the grid/workers or switch to `astropy.timeseries.BoxLeastSquares` for the primary result path.
Use the observed inspection output as the source of truth. Example: if introspection shows the search object has a `power` method, call that method next and inspect its return object once; do not keep trying alternative import names, guessed helper functions, or callable-object patterns.
...
When generating a script file:
- write executable Python immediately, not a natural-language summary of intended steps
- read the saved file back once before execution
- if practical, run `python3 -m py_compile /root/find_period.py` to catch syntax/truncation problems before the full analysis run
- only then execute it end-to-end
```python
import numpy as np
