...
0b. Run a brief preflight before substantive work: confirm the exact required tool/action syntax, output-path policy, and completion token/final-response string; confirm that the current task inputs/artifacts actually contain the needed case facts; identify which helper script, sample record, or schema defines the required machine-readable mapping format; and, if the task or bundled workflow requires certain PDF steps to be run first or in order (for example a fillability check before extraction/filling), execute those prerequisite commands successfully before any downstream mapping or fill attempt. If any of these prerequisites are missing, unreadable, or failed, stop and resolve that first.
...
1a. Treat this fillability decision as the workflow fork: once native fields are confirmed, stay on the structured AcroForm/helper-script path (field extraction -> reviewed mapping file -> fill -> saved-PDF verification) rather than improvising manual editing or overlay methods, and only change approaches if that path is proven unusable.
...
3b. If the extracted field list is saved to a file, inspect the actual saved file beyond a short preview before claiming any mapping is complete. Search/read enough of the full inventory to identify each target field explicitly; do not treat the first few lines, a head output, or a truncated console snippet as review of the schema.
...
3b.i. Do not use plain file reads of PNG/page-image binaries as evidence of visual inspection. Use an image-capable rendering, OCR, or preview method that returns readable page content; if it returns nothing usable, treat layout review as not done.

3c. Default pre-fill sequence for native fillable PDFs: (1) confirm fillability, (2) export the full field inventory/metadata to a file, (3) review rendered page images or page text alongside that inventory, and only then (4) draft the field-values mapping. For large inventories, narrow candidates by page and semantic keywords from the case facts; for opaque or non-semantic field names, use page number plus field rectangles/crops to tie each metadata entry to the visible prompt, checkbox option, or section before mapping values. Do not skip straight from case facts to writing field values.
...
4b. Do not make a first fill attempt until you can name the required top-level shape and per-item keys the helper consumes. A self-designed values-only JSON is not an acceptable trial run.

4c. If a provided script or command fails because the expected runtime/executable is unavailable (for example `python` not found), inspect the environment for the correct available interpreter/tool variant (for example `python3`) and rerun with that exact available executable instead of repeating the same failing command.
...
6b. Before writing any `field_values` or helper-input file, make the mapping explicit for each populated entry: `PDF field ID/name -> visible prompt/section -> source case fact -> value to write`. Do not create the final payload from a partial field dump, placeholder description, or unshown reasoning.
...
8b.i. Hard stop: if the mapping/input artifact is not valid structured data in the exact format expected by the downstream fill step, do not run the fill command. Make the saved mapping evidence visible in the run by inspecting or printing the actual JSON/object contents or an equivalent parsed view. It must deserialize into the expected structure and contain real per-field entries with confirmed identifiers, required metadata, and intended values. Replace placeholder sentences, narrative notes, or status text with the real machine-readable mapping, then read it back again before proceeding.

8d. Treat the intermediate mapping artifact as the handoff between identification and filling: complete and review it before the first fill attempt, and use it to drive both the fill step and later verification rather than remapping ad hoc from memory.
...
9c. Treat missing dependencies or unavailable utilities as part of the same escalation decision: if a candidate workflow depends on tools not installed in the environment, do not keep circling through nearby variants of that workflow. Choose a method that is both structurally sound for this PDF and actually runnable in the current environment.
...
10a. At each major handoff artifact (for example extracted field metadata, mapping JSON, and the filled PDF), do a quick existence/readability check immediately after creation so failures are caught before the next step; then continue to the deeper content verification required elsewhere in this workflow. Treat this as an early guardrail only, not proof that filling succeeded.
...
11b. A process check is never a fill verification step: do not use `ls`, file timestamps, output-path existence, byte size, or a tool's generic success/update message as evidence that any field value was populated. If your only post-fill evidence is process or existence output, or if page-image reads, OCR, or preview commands return empty or unreadable results, treat verification as not done and do not claim specific contents were written.
...
12b. If earlier fill attempts produced structural/document-read errors (for example missing `/AcroForm`, zero fields after fill, blank extracted values, unreadable objects, or broken references), treat any later "successfully filled" print from the same workflow as untrusted until an independent verification method successfully reads the saved PDF and confirms the intended populated fields.

12c. Preferred post-fill verification sequence for fillable PDFs: first read back saved field values when native form fields remain available, then render the saved output PDF and inspect the populated pages for the required/high-risk fields you set. Treat this combination as the default verification path for fillable PDFs.

12d. If one verification command fails because the checker itself is unavailable or errors (for example missing system utilities or object-read failures), discard it as evidence and replace it with a different successful verification path before concluding anything. Do not average together failing checks and one later summary into a claim of success.
...
14b. Use a final-response evidence gate: before the last turn, confirm (1) at least one successful verification step showed the filled content itself or programmatic read-back of the saved PDF, and (2) every specific fact you mention in the response, including names, parties, amounts, dates, allegations, addresses, filing basis, checkbox selections, and field mappings, was visibly supported by both a source input and the inspected mapping or verified output from this run. If either check fails, do not summarize populated details; leave them unconfirmed until a successful inspection path resolves them.



15. Before the final response, check whether the task specifies an exact completion token, final response string, or output protocol; if so, compare your planned final output literally against that requirement and emit that required text exactly, with no extra prose before or after it. Do not replace it with a narrative summary, even if the work appears complete.


16. Use this default path for native-form PDFs unless the task requires another validated workflow: confirm the PDF is fillable -> extract the full field inventory/metadata -> map confirmed case facts to specific field records -> fill -> read back key written fields from the saved PDF -> fix any omissions/mismatches -> verify again before finalizing.

16a. During read-back verification, include the highest-risk populated fields first: party names, dates, addresses/contact fields, amounts, long text/narrative boxes, and every checkbox/radio selection you set. If read-back shows a missing required field, return to the mapping and fill steps rather than assuming the rest succeeded.
...
- When a task or bundled PDF instructions require prerequisite commands such as a fillability/schema check, run those exact prechecks first and use their results to choose the fill method; do not skip ahead to extraction or filling.
- Empty PNG/image reads mean you have not actually reviewed the page; if multiple page/image reads are blank, explicitly treat visual inspection as failed and switch to another inspection method before claiming layout understanding or that the output looks correct.
- Reading only a truncated preview of an extracted field file is not enough to support a complete mapping claim; inspect the saved schema file itself far enough to confirm every field you plan to populate.
- A created helper-input file is not sufficient by itself; inspect the saved contents and confirm it is real structured data in the exact expected schema, not placeholder prose or status text, before any dependent fill step.
- Do not claim that case facts were mapped into the form unless the run shows the actual field identifiers from the extracted schema paired with the source facts and intended values.
- If prior attempts showed structural corruption or unreadable outputs, a later helper/script success print from that same path is still only a process signal; require independent read-back or rendered verification of the saved PDF before accepting the result.
- Do not include a final narrative claiming specific filled details unless those same details appear in the inspected mapping artifact or verified read-back/render output from the saved PDF.
- If the environment requires an `Action:`-style or other exact tool wrapper, use that exact syntax on every tool step; do not switch to alternate call styles mid-run.
- If the task says to use absolute paths as provided, use absolute paths consistently for all reads, writes, commands, and final reported outputs.
- Make the exact required completion token/string the entire final response with no prefatory or trailing explanation.
...
- Proven default sequence for fillable PDFs: confirm fillability, export field metadata, inspect rendered pages to align field IDs with visible labels, build and inspect an intermediate mapping JSON, then fill and verify against that mapping.
- For large or cryptic field inventories, narrow candidates by page and case-related keywords, then use page number plus field rectangle/crop evidence to identify the exact target instead of relying on raw field names alone.
- Treat the intermediate mapping file as the reviewable handoff between identification, filling, and verification; inspect the saved mapping before the first fill attempt and compare later verification against it.
- If a helper script error names a missing key such as `page`, treat that as proof your mapping schema is wrong; stop, inspect the script or sample data, regenerate the full per-field object structure, and reread the saved JSON before retrying.
- If image reads/previews return empty output, stop saying the layout was reviewed; switch to OCR, rendering through another tool, or metadata-plus-coordinate inspection before mapping fields.
- If a provided helper script fails immediately because the named executable is missing, check which interpreter/tool variant is actually installed (for example `python3` vs `python`) before retrying.
- If a task requires an exact completion token such as `ACTION: TASK_COMPLETE`, treat that token as the entire final response; do not prepend or append a prose summary even when verification succeeded.
