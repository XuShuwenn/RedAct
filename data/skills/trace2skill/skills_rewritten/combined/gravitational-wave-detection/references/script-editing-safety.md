...
6. Base replacements on exact source lines copied from the current file; do not patch against descriptive stand-ins or imagined code.
7. After editing, run `python -m py_compile <script>` or an equivalent syntax/import check before any long or background execution.
