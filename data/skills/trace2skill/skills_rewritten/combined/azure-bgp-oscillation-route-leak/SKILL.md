...
- Do not switch to guessed alternate filenames (for example changing an observed `.jso` name to `.json`) or proceed on intended names; first open the observed file that actually supplies that dataset.
...
- Do not claim oscillation, route leaks, or solution outcomes from files that were listed but not actually opened and inspected. Required conclusions must be traceable to specific fully read inputs.
...
14a. Do not claim that a script, report, or solution evaluation exists unless its substantive contents are visible or directly validated in-session.
14b. If any output readback is truncated, cut off mid-field, or hidden behind a placeholder or summary, treat the analysis as unverified and re-read the artifact before making conclusions.
...
15a. Do not delegate the end-to-end evaluation of candidate solutions. If any helper is used for partial assistance, recompute each solution's `oscillation_resolved` and `route_leak_resolved` values yourself from the loaded inputs before writing the report.
15b. Treat delegated analytical output as hints only; do not copy solution judgments, oscillation findings, or leak findings into the final JSON until you can point to the specific input evidence and normalized logic supporting each one.
...
19. If any verification readback is truncated, cut off mid-JSON, omits any solution entry or closing structure, or shows status text/commentary instead of the serialized report object, treat verification as failed: re-read or rewrite the full file and validate again before continuing.
20. Do not claim that solutions were evaluated, or state exact counts, rankings, or recommendations, unless the interaction log shows the full `possible_solutions.json` was read and the per-solution results were derived from that complete input.
21. Before the final response, run a termination check only after complete readback/validation succeeds: confirm the output file is fully written and validated, reconcile the confirmed solution list with the saved `solution_results` keys and count, and confirm the final response itself matches the exact task-required completion format/token with identical spelling, punctuation, and casing.

22. Do not use a single opaque command or summary step that claims to have loaded files, analyzed topology, evaluated all solutions, and written the report at once. Make the workflow inspectable: show the file reads, normalization, issue detection, and per-solution evaluation in separate visible steps or clearly scoped commands.
23. Keep the execution trail observable: list files with a real command, read each required input with real commands, write the report with a real command, then read back the saved file with a real command before claiming completion.
24. After any normalization change, audit every downstream consumer before continuing; do not leave cycle detection, leak checks, or solution evaluation reading the pre-normalized structure.
25. When you claim the inputs are consistent, valid, or sufficient, make the supporting checks observable in the log first: show concrete cross-checks such as matching ASNs across files, confirming relationship labels and directions, and tracing the relevant advertised path against the policy rule used.
26. Keep the evaluated solution set auditable while you work: record the exact confirmed list from `possible_solutions.json`, and ensure the final saved `solution_results` covers that same set completely with no dropped item, duplicate collapse, or summary-only evaluation.
...
- Write the serialized report object itself. Do not write status text such as `saved the completed JSON report`, placeholders, summaries, or any other non-JSON stand-in for the artifact.
...
- Construct the full report object in memory first, then write that serialized JSON object to `/app/output/oscillation_report.json`; never write status text such as `saved the completed JSON report` or any other placeholder in its place.
- Before claiming the report was saved, read back the file and confirm the on-disk bytes are the same structured JSON deliverable you intend to submit, not merely a note that saving occurred.
...
- Keep final claims evidence-bounded: do not state counts, per-solution judgments, classifications, or completion of full evaluation unless those exact results were fully read back from the saved report or otherwise directly verified from complete tool output.
...
- Treat placeholder or status-string writes as task failure. The saved file must be the actual JSON report object you derived.
- Before final completion, reconcile the same confirmed solution list across three places: the fully loaded `possible_solutions.json` contents, the in-memory evaluation, and the saved `solution_results` keys and count.
- Do not treat a reasoning note or summary table as sufficient; the persisted JSON must contain every evaluated solution entry.
...
- Sparse evidence is not permission to fill in Azure-specific topology details: if the files show only a cycle and no explicit violating advertisement/policy pair, report oscillation only and leave `route_leak_detected` false.
- For each solution marked effective, be able to name the specific cycle edge it breaks or the specific prohibited advertisement it blocks from the observed inputs.
- Do not mark a solution effective for one issue merely because it helps with another; a route-filtering measure does not resolve oscillation unless it breaks the observed preference cycle, and a preference change does not resolve a leak unless it blocks the evidenced prohibited advertisement.
- Treat solution names as labels, not proof. Do not mark a solution effective just because it sounds like filtering, routing intent, export control, or traffic steering unless the provided files state the relevant effect.
- Operational correctness is mandatory: even a correct report fails the task if the required action syntax or exact completion token is not followed literally.