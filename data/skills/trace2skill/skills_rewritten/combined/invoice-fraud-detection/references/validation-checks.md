...
- Do not substitute marker counts (for example page headers found, PO labels found, or regex hit counts) for per-page extraction review. Counts can match even when some pages still contain truncated values such as partial PO numbers or clipped IBANs.
...
- If you edit the analysis script after any earlier validation, discard those earlier report conclusions. Regenerate `/root/fraud_report.json`, then repeat full-file parsing plus representative source-backed checks before stating counts or correctness.
...
- Include at least one contradiction hunt during final review: pick a flagged invoice from a lower-priority category and manually/programmatically check all earlier rules in order against the exact matched vendor and PO records. Do not finalize if the stored reason is not the earliest applicable one.
- If you generate a `not flagged pages`, missing-pages, or reconciliation list during validation, resolve every page on that list before finalizing. Once any page's status changes after investigation, regenerate `/root/fraud_report.json` and recompute the list from the new file.
- Do not treat a manual fuzzy-match experiment or direct lookup for one invoice as proof that the final report used that logic. Re-run the production script and verify the same page in the saved JSON.
- Add boundary-case review after the full run: inspect at least one unflagged invoice, one accepted non-exact vendor-name match, and one rejected borderline or ambiguous vendor-name match.
- When a prior read of `purchase_orders.csv` or another reference source was truncated, explicitly inspect targeted late-file records or the file tail before concluding a PO or vendor is absent.
- For representative checks, validate the full chain together: matched vendor identity, canonical IBAN, PO presence, PO-linked Vendor ID, and amount agreement. This helps catch cases where one signal looks correct but another should still trigger the first-applicable fraud reason.
