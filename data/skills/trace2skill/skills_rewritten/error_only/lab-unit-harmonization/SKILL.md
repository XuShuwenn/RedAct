...
2b. Before applying any destructive step from this skill, especially row deletion, confirm that the current task/environment does not override this default requirement and that parsed-data evidence supports the deletion.
...
Use the task-specified output location if one is provided and it differs from the example below. Do **not** write to `/root/` or any other disallowed directory when the environment restricts file access.
