...
4. **Closed-loop Control**: Run to 22.0°C → `control_log.json` + `metrics.json`
   ...
   - Treat visibly cut-off output, startup-only text, mid-phase logs, or partial file prints as non-evidence. If output is incomplete or ends mid-message, performance is unknown until you confirm the run completed and directly read the saved artifacts from that same run.
