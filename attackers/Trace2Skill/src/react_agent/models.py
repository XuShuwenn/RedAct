...
def _is_unsupported_param_error(exc: Exception, param_name: str) -> bool:
    """Return True when an OpenAI-compatible provider rejects one parameter."""
    body = getattr(exc, "body", None)
    param = None
    code = None
    if isinstance(body, dict):
    ...
    return (
        param == param_name
        or ("unsupported parameter" in message and f"'{param_name}'" in message)
        or (code == "unsupported_parameter" and param_name in message)
    )
    ...
    def _send_request_with_retry(self, messages: list[dict], config: dict):
        ...
        for i, wait_time in enumerate(self.retry_times):
            try:
                ...
                if "stop" in config and _is_unsupported_param_error(e, "stop"):
                    log.warning("Provider rejected stop sequences; retrying without stop.")
                    config = dict(config)
                    ...
                    try:
                        ...
                        if _is_context_length_bad_request(e):
                            raise RequestContextLengthExceeded(
                                _extract_openai_error_message(e)
                            ) from e
                log.warning(
