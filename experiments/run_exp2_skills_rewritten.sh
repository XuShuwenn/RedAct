...
# Experiment 2: 6 agent+model combos on 75 tasks with skills_rewritten
# Skills from: data/skills/skill_extract/skills_rewritten
...
SKILLS_BASE="${ROOT_DIR}/data/skills/skill_extract/skills_rewritten"
OUTPUT_BASE="${ROOT_DIR}/results/exp2_skills_rewritten"
...
JOB_FILE="$(mktemp "${TMP_DIR}/exp2_jobs.XXXXXX")"
...
while [[ "$chunk_start" -le "$total" ]]; do
  ...
  chunk_file="$(mktemp "${TMP_DIR}/exp2_chunk.XXXXXX")"
