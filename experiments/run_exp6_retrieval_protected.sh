...
# Experiment 6: retrieval_protected - tasks from tasks_rag/protected with rewritten instructions, no skills
...
TASKS_BASE="${ROOT_DIR}/tasks_rag/protected/cross_check"
OUTPUT_BASE="${ROOT_DIR}/results/exp6_retrieval_protected/cross_check"
...
echo "Tasks base: $TASKS_BASE (no skills, protected instructions)"

BATCH_ID="$(date +%Y-%m-%d__%H-%M-%S)"
LOG_DIR="${OUTPUT_BASE}/_batch_logs/${BATCH_ID}"
mkdir -p "$LOG_DIR"

JOB_FILE="$(mktemp "${TMP_DIR}/exp6_jobs.XXXXXX")"
...
while [[ "$chunk_start" -le "$total" ]]; do
  ...
  chunk_file="$(mktemp "${TMP_DIR}/exp6_chunk.XXXXXX")"
