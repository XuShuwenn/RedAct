...
# Experiment 4: 6 agent+model combos on 75 tasks with trace2skill/skills_rewritten/combined
# Skills from: data/skills/trace2skill/skills_rewritten/combined
...
SKILLS_BASE="${ROOT_DIR}/data/skills/trace2skill/skills_rewritten/combined"
OUTPUT_BASE="${ROOT_DIR}/results/exp4_trace2skill_rewritten_combined"
...
JOB_FILE="$(mktemp "${TMP_DIR}/exp4_jobs.XXXXXX")"
...
while [[ "$chunk_start" -le "$total" ]]; do
  ...
  chunk_file="$(mktemp "${TMP_DIR}/exp4_chunk.XXXXXX")"
