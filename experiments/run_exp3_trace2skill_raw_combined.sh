...
# Experiment 3: 6 agent+model combos on 75 tasks with trace2skill/skills_raw/combined
# Skills from: data/skills/trace2skill/skills_raw/combined
...
SKILLS_BASE="${ROOT_DIR}/data/skills/trace2skill/skills_raw/combined"
OUTPUT_BASE="${ROOT_DIR}/results/exp3_trace2skill_raw_combined"
...
JOB_FILE="$(mktemp "${TMP_DIR}/exp3_jobs.XXXXXX")"
...
while [[ "$chunk_start" -le "$total" ]]; do
  ...
  chunk_file="$(mktemp "${TMP_DIR}/exp3_chunk.XXXXXX")"
